...
/**
 * Created by dwang on 4/25/17.
 */
public class ConfigurableSFTPTransfer implements FileTransfer {
    ...
    public static final PropertyDescriptor CONFIG_SCOPE=new PropertyDescriptor.Builder()
            .name("Configuration Scope")
            .description("The configuration scope to look up properties, used to get property for scope.properties")
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .dynamic(false)
            .expressionLanguageSupported(true)
            .defaultValue(null)
    ...
    public static final PropertyDescriptor ROTATE_COUNT = new PropertyDescriptor.Builder()
            .name("Rotate Count")
            .required(false)
    ...
    public static final PropertyDescriptor PROPERTIES_FILE_SERVICE =new  PropertyDescriptor.Builder()
            .name("PropertiesFileService Controller")
            .description("PropertiesFileService Controller Service")
    ...
    private void getListing(final String path, final int depth, final int maxResults, final List<FileInfo> listing) throws IOException {
        ...
        try {
            final ChannelSftp.LsEntrySelector filter = new ChannelSftp.LsEntrySelector() {
                @Override
    ...
    public String put(final FlowFile flowFile, final String path, final String filename, final InputStream content) throws IOException {
        ...
        // check keepCount if it's larger than 0, archive file
        int keepCount = ctx.getProperty(ROTATE_COUNT).asInteger();
        if(keepCount>0) {
            archiveFile(path, filename, keepCount );
        }
    ...
    static {
        JSch.setLogger(new com.jcraft.jsch.Logger() {
            ...
            public void log(int level, String message) {
                LoggerFactory.getLogger(ConfigurableSFTPTransfer.class).debug("SFTP Log: {}", message);
            }
        });
    }

    protected  void archiveFile(final String path, final String remoteFileName, int keepCount){

        logger.info("Archive remote file {} in {}", new Object[]{remoteFileName, path});
        if(keepCount<1) {
            logger.info("Rotate count is less than 1, skip archive.");
            return;
        }
        ...
        try {
            ...
            this.deleteFile(path, remoteFileNameDelete);
        }catch (FileNotFoundException e){
            logger.warn("Error while delete file: {}", new Object[]{e.getMessage()});
        } catch (IOException e) {
            logger.warn("Cannot delete remote file: " +e.getMessage());
        }
        ...
        for(int i=keepCount-1; i>=0; i--) {
            ...
            logger.info("Rename file from {} to {}", new Object[]{oldFullPath, newFullPath});
            try {
                this.rename(oldFullPath, newFullPath);
            }catch (FileNotFoundException e){
                logger.warn("Cannot found file renaming: {}.", new Object[]{e.getMessage()});
            } catch (IOException e) {
                logger.warn("Cannot rename file: " +e.getMessage());
            }
        }
    }
}
